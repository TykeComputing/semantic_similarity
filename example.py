...
###################### NOTE FROM MATTHEW ######################
'''
When comparing the usage and output of en_core_web_md to en_core_web_sm, the program
runs with a 'UserWarning: [W007]', which notes that the small model en_core_web_sm
doesn't use word vectors, thus providing a much lower comparison between sentences
of similar themes than the larger en_core_web_md package.
'''